...
struct Quaternion final
    ...
    constexpr Quaternion(const Vec3f& v, float w)
        : Vector<4, float>(v[X], v[Y], v[Z], w) {}
    ...
    /// 2つのベクトルの間の回転を作成
    /// </summary>
    static const Quaternion RotateAxisVector(const Vec3f& from, const Vec3f& to);

    /// <summary>
    /// 法線ベクトルと上方向ベクトルからクォータニオンを作成
    /// </summary>
    static const Quaternion FromNormalVector(const Vec3f& normal, const Vec3f& up);

    /// <summary>
    /// 行列からクォータニオンを作成
    /// </summary>
    static Quaternion FromMatrix(const Matrix4x4& rotateMat);
    /// <summary>
    /// オイラー角からクォータニオンを作成
    /// </summary>
    ...
    /// 前方向と上方向からクォータニオンを作成
    /// </summary>
    static Quaternion LookAt(const Vec3f& forward, const Vec3f& up);
};
