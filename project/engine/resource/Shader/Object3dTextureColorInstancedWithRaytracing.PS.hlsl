...
///========================================
StructuredBuffer<Material> gInstanceMaterials : register(t7);
...
PixelShaderOutput main(InstancedVertexShaderOutput input)
{
    ...
    Material mat = gInstanceMaterials[input.instanceId];

    // texcoord を z=0 の (3+1)次元 として考える
    float4 transformedUV = mul(float4(input.texCoord, 0.0f, 1.0f), mat.uvTransform);
    float4 textureColor = mat.color * gTexture.Sample(gSampler, transformedUV.xy);
    ...
    float3 objectColor = textureColor.rgb * input.color.rgb;

    if (mat.enableLighting == 0)
    {
     ...
     ++directionalLightIndex)
    {
        ...
        float specularPow = pow(saturate(NdotH), mat.shininess);
     ...
     ++pointLightIndex)
    {
        ...
        float3 specular = lightColor * specularPow * mat.specularColor;
    ...
    output.color.rgb += environmentColor.rgb * mat.environmentCoefficient;
