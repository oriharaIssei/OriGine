...
std::unique_ptr<Animation> AnimationManager::Load(const std::string& directory,const std::string& filename){
    ...
    if(animationIndex != animationDataLibrary_.end()){
        result->data = animationData_[animationIndex->second].get();
    } else{
        // 新しい ポインタを作成
        ...
        result->data = animationData_.back().get();

        ///===========================================
        /// TaskThread に ロードタスクを追加
        ///===========================================
        AnimationLoadTask task(directory,filename,result->data,result.get());
