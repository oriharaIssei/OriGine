...
void Emitter::Debug() {
    if (ImGui::Begin(emitterName_.c_str())) {
        ...
        ImGui::DragFloat3("##Emitter Origin", reinterpret_cast<float*>(originPos_.operator Vec3f*()), 0.1f);
        ...
        ImGui::DragFloat3("##Particle minDirectory", reinterpret_cast<float*>(minDirectory_.operator Vec3f*()), 0.1f);
        ImGui::Text("Particle maxDirectory");
        ImGui::DragFloat3("##Particle maxDirectory", reinterpret_cast<float*>(maxDirectory_.operator Vec3f*()), 0.1f);

        minDirectory_.setValue(
            {(std::min)(minDirectory_->x(), maxDirectory_->x()),
             (std::min)(minDirectory_->y(), maxDirectory_->y()),
             (std::min)(minDirectory_->z(), maxDirectory_->z())});

        maxDirectory_.setValue(
            {(std::max)(minDirectory_->x(), maxDirectory_->x()),
...
void Emitter::SpawnParticle() {
    ...
    for (int32_t i = 0; i < canSpawnParticleValue_; i++) {
        ...
        spawnedParticle->Init(initialTransform, velocity, particleLifeTime_);
    }
}
