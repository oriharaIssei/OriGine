...
struct Matrix4x4 {
    ...
    /// </summary>
    /// <param name="mat">抽出元の行列</param>
    /// <returns>抽出した回転成分</returns>
    static Quaternion DecomposeMatrixToQuaternion(const Matrix4x4& mat);
    /// <summary>
    /// 行列から回転成分をQuaternionで抽出
    /// </summary>
    ...
    /// </summary>
    static DirectX::XMMATRIX MatrixToXMMATRIX(const Matrix4x4& mat) {
    ...
    /// </summary>
    static Matrix4x4 XMMATRIXToMatrix(const DirectX::XMMATRIX& xmmat) {
        Matrix4x4 result{};
        XMStoreFloat4x4(reinterpret_cast<DirectX::XMFLOAT4X4*>(&result), xmmat);
        return result;
    }
    void xmmatrixToMatrix(const DirectX::XMMATRIX& xmmat) {
        *this = XMMATRIXToMatrix(xmmat);
    }
};
