...
void GlobalVariables::LoadFile(const std::string& _scene, const std::string& _groupName) {
    std::string dir = kDirectoryPath + _scene + "/" + _groupName + ".json";
    ...
    json::iterator groupItr = root.find(_groupName);
    ...
    for (; itemItr != groupItr->end(); ++itemItr) {
        ...
        if (itemItr->is_number_integer()) {
            ...
            SetValue(_scene, _groupName, itemName, value);
        } else if (itemItr->is_number_float()) {
            // float なら
            double value = itemItr->get<double>();
            float fValue = static_cast<float>(value);
            SetValue(_scene, _groupName, itemName, fValue);
...
#pragma region "Save"
void GlobalVariables::SaveScene(const std::string& _scene) {
    for (auto& group : data_[_scene]) {
        std::string dir = kDirectoryPath + _scene + "/" + group.first + ".json";
        SaveFile(_scene, group.first);
    }
}

void GlobalVariables::SaveFile(const std::string& _scene, const std::string& _groupName) {
    ///========================================
    /// 保存項目をまとめる
    ///========================================
    std::map<std::string, Group>::iterator groupItr = data_[_scene].find(_groupName);
    assert(groupItr != data_[_scene].end());
    ...
    for (std::map<std::string, Item>::iterator itemItr = groupItr->second.begin();
        ...
        if (std::holds_alternative<int32_t>(item)) {
            root[_groupName][itemName] = std::get<int32_t>(item);
        } else if (std::holds_alternative<float>(item)) {
            root[_groupName][itemName] = std::get<float>(item);
        } else if (std::holds_alternative<std::string>(item)) {
            root[_groupName][itemName] = std::get<std::string>(item);
        } else if (std::holds_alternative<Vec2f>(item)) {
            Vec2f value                = std::get<Vec2f>(item);
            root[_groupName][itemName] = json::array({value[X], value[Y]});
        } else if (std::holds_alternative<Vec3f>(item)) {
            Vec3f value                = std::get<Vec3f>(item);
            root[_groupName][itemName] = json::array({value[X], value[Y], value[Z]});
        } else if (std::holds_alternative<Vec4f>(item)) {
            Vec4f value                = std::get<Vec4f>(item);
            root[_groupName][itemName] = json::array({value[X], value[Y], value[Z], value[W]});
        } else if (std::holds_alternative<bool>(item)) {
            root[_groupName][itemName] = std::get<bool>(item);
        }

        ///========================================
        /// ファイルへ書き出す
        ///========================================
        std::string path = kDirectoryPath + _scene + "/";
        std::filesystem::path dir(path);
        if (!std::filesystem::exists(dir)) {
            std::filesystem::create_directory(dir);
        }
        std::string filePath = path + _groupName + ".json";
