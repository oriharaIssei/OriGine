...
template <typename T, int N>
bool InputVectorGui(const std::string& label, T (&value)[N], const char* format = "%.3f") {
    if constexpr (std::is_same_v<T, int>) {
        if constexpr (N == 2)
...
//         EditorGroup::getInstance()->pushCommand(std::make_unique<SetterComamnd<T>>(value, &newValue));
//         return true;
//     }
//     return false;
// }
...
//     if (DragGui(label, newValue, speed, min, max, format)) { // 値が変更された場合
//         EditorGroup::getInstance()->pushCommand(std::make_unique<SetterComamnd<T[N]>>(value, &newValue));
//         return true;
//     }
//     return false;
// }
...
// 原則 Inputのみ 使用する
template <typename T>
bool InputGuiCommand(const std::string& label, T& value, const char* format = "%.3f") {
    T newValue = value; // 現在の値をコピー
...
template <typename T, int N>
bool InputVectorGuiCommand(const std::string& label, T (&value)[N], const char* format = "%.3f") {
    T newValue[N]; // 現在の値をコピー
    std::copy(value, value + N, newValue);
    if (InputVectorGui(label, newValue, format)) { // 値が変更された場合
        for (size_t i = 0; i < N; i++) {
            if (value[i] != newValue[i]) {
                EditorGroup::getInstance()->pushCommand(std::make_unique<SetterComamnd<T>>(&value[i], newValue[i]));
            }
        }
        return true;
    }
