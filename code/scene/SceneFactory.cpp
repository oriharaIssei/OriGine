...
bool SceneFactory::BuildSceneByName(Scene* _scene,const std::string& _sceneName){
	...
	if(!json){
		...
		// loadを試みる
		registry->LoadScene(_sceneName,kApplicationResourceDirectory + '/' + std::string(kSceneJsonFolder));
		json = registry->GetSceneJson(_sceneName);
		if(!json){
			LOG_ERROR("BuildSceneByName: JSONのロードに失敗しました: {}",_sceneName);
			return false;
		}
	}
