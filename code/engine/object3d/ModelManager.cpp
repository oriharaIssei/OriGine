...
void ProcessMeshData(ModelData& modelData,const std::vector<TextureVertexData>& vertices,const std::vector<uint32_t>& indices){
	...
	memcpy(modelData.meshData.meshBuff->indexData,indices.data(),static_cast<size_t>(static_cast<size_t>(indices.size())) * sizeof(uint32_t));
...
void LoadObjFile(Model* model,const std::string& directoryPath,const std::string& filename){
	...
	for(uint32_t meshIndex = 0; meshIndex < scene->mNumMeshes; ++meshIndex){
		model->data_.emplace_back(ModelData());
		...
		if(material->GetTexture(aiTextureType_DIFFUSE,0,&textureFilePath) == AI_SUCCESS){
			...
			model->data_.back().materialData.textureNumber = TextureManager::LoadTexture(texturePath);
		}

		// マテリアル名の設定（仮）
		model->data_.back().materialData.material = System::getInstance()->getMaterialManager()->Create("white");

		// メッシュデータを処理
		ProcessMeshData(model->data_.back(),vertices,indices);
...
void ModelManager::LoadTask::Update(){
	...
	LoadObjFile(model,this->directory,this->fileName);
